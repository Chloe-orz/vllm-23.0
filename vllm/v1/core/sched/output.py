...
@dataclass
class LwdEmbedBatch:
    """LWD prefill_only mode edge execute batch"""
...
class NewRequestData:
    ...
    # LWD 边云:无 ids 也无 embeds 张量时的显式长度载体(替代此前随
    # SO 运输 35MB 零占位 buffer;长度经此直达 worker,各 rank 不再
    # 创建任何占位 buffer)。普通请求恒为 None,长度仍按原逻辑推导。
...
class SchedulerOutput:
    ...
    # LWD batch metadata dispatch to the worker.
    lwd_batch: LwdBatch | None = None
