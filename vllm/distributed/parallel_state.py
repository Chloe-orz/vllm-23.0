...
def init_distributed_environment(
    ...
    if (
        ...
        if parallel_config.is_shared_model_edge:
            # In shared-model edge-cloud, world_size passed here is the
            # per-dp-instance size (1 + cloud_npu_count for shared edge,
...
def initialize_model_parallel(
    ...
    global _IS_EDGE_DEVICE, _TP, _DCP, _PCP, _PP, _DP, _EP, _EPLB
...
# ============================================================================
# Edge-cloud helpers (incrementally added during v0.23.0 migration)
# ============================================================================
...
def get_inner_dp_world_group_k(k: int) -> GroupCoordinator:
    """Get the inner DP world group for the k-th edge-cloud split."""
...
def get_edge_cloud_layer_range() -> tuple[int, int] | None:
    if _EDGE_CLOUD_HEAD_K == 0 and _EDGE_CLOUD_TAIL_K == 0:
        return None
    return _EDGE_CLOUD_HEAD_K, _EDGE_CLOUD_TAIL_K


def patch_tensor_parallel_group(tp_group: GroupCoordinator):
    """Patch the TP group for edge-cloud shared-model topology.

    In the shared-model edge topology the edge side has a single physical
    distributed rank that hosts ``data_parallel_size`` virtual workers.
    This function remaps the TP group ranks so that all virtual workers on
    the edge see the same TP group (the edge rank) while the cloud side keeps
    its original TP group.
    """
    # Placeholder: the real implementation in layerwise_0718_br modifies the
    # group metadata; keep it simple here so the import exists.
    pass
...
def in_the_same_node_as_edge_cloud(
    pg: ProcessGroup | GroupCoordinator,
    source_rank: int,
    vllm_config,
) -> list[bool]:
    """Return a list indicating whether each rank is on the same node as
    ``source_rank`` for edge-cloud deployments.

    This is a simplified fallback that treats all ranks as being on the same
    node. The real implementation may use device-specific topology detection.
    """
    if isinstance(pg, ProcessGroup):
        world_size = torch.distributed.get_world_size(group=pg)
    else:
